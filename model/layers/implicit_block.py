...
def anderson(f, x0, m=5, lam=1e-4, max_iter=50, tol=1e-2, beta=1.0):
    ...
    bsz = x0.shape[0]
    flatten = int(x0.numel()/bsz)
    X = torch.zeros(bsz, m, flatten, dtype=x0.dtype, device=x0.device)
    F = torch.zeros(bsz, m, flatten, dtype=x0.dtype, device=x0.device)
    ...
    for k in range(2, max_iter):
        ...
        try:
            ...
            print(e)
            alpha = torch.column_stack([torch.zeros(
                (bsz, n-1), device=x0.device), torch.ones((bsz, 1), device=x0.device)])
...
def find_fixed_point(g, y, threshold=1000, eps=1e-5):
    ...
    while not torch.all((x - x_prev)**2 / tol < 1.):
        ...
        if i > threshold:
            ...
            logger.info(f'Iterations exceeded {threshold} for fixed point.')
            break
    return x
...
class RootFind(Function):
    ...
    @staticmethod
    def anderson_find_root(nnet_z, nnet_x, z0, x, *args):
        eps = args[-2]
        # Can also set this to be different, based on training/inference
        threshold = args[-1]
        x_embed = nnet_x(x) + x
        def g(z): return x_embed - nnet_z(z)
        z_est = anderson(g, z0, max_iter=threshold, tol=eps, beta=1)
        if threshold > 100:
            torch.cuda.empty_cache()
        return z_est.clone().detach()
    ...
    def forward(ctx, nnet_z, nnet_x, z0, x, method, *args):
        ...
        if method == 'anderson':
            root_find = RootFind.anderson_find_root
        else:
...
class imBlock(nn.Module):
    ...
    class Backward(Function):
        ...
        @staticmethod
        def backward(ctx, grad, method=METHOD):
            torch.cuda.empty_cache()
            ...
            if method == 'broyden':
                ...
                dl_dh = anderson(g, dl_dh, m=5, max_iter=threshold, tol=eps)
    ...
    def forward(self, x, logpx=None, restore=False, method=METHOD):
        z0 = x.clone().detach()
        if restore:
            with torch.no_grad():
        ...
        z = RootFind.apply(self.nnet_z, self.nnet_x, z0, z0,
                           method, self.eps_forward, self.threshold)
        ...
        z = self.Backward.apply(self.nnet_z_copy, self.nnet_x_copy,
                                z, x, method, self.eps_backward, self.threshold)
        if logpx is None:
            return z
        else:
            return z, logpx - self._logdetgrad(z, x)

    def inverse(self, z, logpy=None, method=METHOD):
        x0 = z.clone().detach()
        x = RootFind.apply(self.nnet_x, self.nnet_z, x0, z,
                           method, self.eps_sample, self.threshold)
